...
(function() {
  var DefaultDel, DefaultGet, DefaultPost, DefaultPut, MWareTransformer, ResourceDomain;

  DefaultGet = require("" + __dirname + "/defaults/resource/get");

  DefaultPut = require("" + __dirname + "/defaults/resource/put");

  DefaultPost = require("" + __dirname + "/defaults/resource/post");

  DefaultDel = require("" + __dirname + "/defaults/resource/del");
  ...
  */
  ...
  ResourceDomain = (function() {
    ...
    */
    ...
    function ResourceDomain(express, config, conditioner) {
      this.__e = express;
      this.__c = config;
    ...
    ResourceDomain.prototype.enrich = function() {
      var afterHooks, args, beforeHooks, clbk, fn, r, routeHandler, routeKeyedAfter, routeKeyedBefore, _i, _len, _ref, _ref1, _results,
        _this = this;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        args = [routeHandler.route].concat(this.__cond.get(this.__c.domain)).concat(_.filter(_.map(beforeHooks, function(n) {
          return _this.__c.m[n];
        }), function(z) {
          return _.isFunction(z);
        })).concat([routeHandler.handler]).concat(_.filter(_.map(afterHooks, function(n) {
          return _this.__c.m[n];
        }), function(z) {
          return _.isFunction(z);
        })).concat([clbk]);
